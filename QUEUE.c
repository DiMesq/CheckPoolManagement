/* File that defines a QUEUE (FIFO method) */
 ...
 * as needed.
 * It takes as input the pointer to the node being freed (type Link).
 */
 ...
 */
 #define emptyQueue(A) ((A)->head == NULL)
 ...
 * Returns the pointer to the new node.
 */
 Link newNode(void* i, Link nextNode){
 	Link n = (Link) xmalloc(sizeof(struct nodeQ)); /* allocate memory for the node */
 ...
 */
 void addQueue(void* i, Queue q){
 	Link node = newNode(i, NULL); /* creating a new node pointing to NULL */
 ...
 * (FIFO method) and so actualizes it's the queues head.
